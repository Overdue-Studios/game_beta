...
func _drag_start(arg1):
	dragged_item = arg1
	print("drag start = ", dragged_item)

func _drag_stop(arg1):
	print("drag stopped")

	# Ensure hovered_slot and dragged_item are valid
	if hovered_slot is int and hovered_slot >= 0 and hovered_slot < Inv.size() and dragged_item:
		print(Inv, Inv[hovered_slot], hovered_slot)
		...
		var dragged_item_index = extract_number_from_node_name(str(dragged_item))-1
		
		# Swap items in inventory
		var temp = Inv[hovered_slot]
		Inv[hovered_slot] = Inv[dragged_item_index]
		...
		if dragged_item:
			dragged_item.get_children()[0].reparent(get_children()[0].get_children()[hovered_slot])
			print("Reparented item to slot:", hovered_slot)
			center_node_in_parent(get_children()[0].get_children()[hovered_slot].get_children()[0], get_children()[0].get_children()[hovered_slot])
