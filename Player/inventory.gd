...
func add_item(item_name, quantity):
	...
	if item.stackable:
		for i in range(_items.size()):
			...
			var inventory_item = _items[i]
			if inventory_item != null and inventory_item.item_reference.name != item.name:
				continue
			
			if inventory_item != null and  inventory_item.quantity < max_stack_size:
				var original_quantity = inventory_item.quantity
	...
	while remaining_quantity > 0 and _items.size() < 35:
		var index_num = 0
		for i in range(_items.size()):
			#print(_items[i])
